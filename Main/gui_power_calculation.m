...
function [abs_power rel_power] = gui_power_calculation(BNCT,raw,tasklistraw,CalcPowerFeatures,phenotypelistraw)
...
for subject = 1:1:subjects
 ...
 %           chn_clust = PowerFeatures(a,5:end);
   %     else
            ...
            chn_clusttemp = CalcPowerFeatures{a}(6:end);            
     %   end
    
    %LOAD DATA
    EEG = pop_loadset(strcat(raw(subject,1),'\',raw(subject,2)));
    
    switch BNCT.chanord.method
        case 'orig'
            chn_clust = chn_clusttemp;  
        case 'new'
            for i = 1:1:EEG.nbchan
                chnlabels{i} = EEG.chanlocs(1,i).labels;
            end
            for a_ch = 1:size(chn_clusttemp,2)
                label = BNCT.chanord.new.labels{chn_clusttemp(a_ch)};
                
                chn_clust(a_ch) = strmatch(label,chnlabels);
        ...
        if strfind(raw{subject,7},phenotypelistraw{b}) > 0
            for k = 1:1:length(tasklist)
                if strfind(raw{subject,2},(tasklistraw{k})) > 0
                    ...
                    if a == size(CalcPowerFeatures,1) %Checking how many values are already in a given pheno/task to place next value appropriately
